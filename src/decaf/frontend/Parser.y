...
%token LITERAL
%token IDENTIFIER	  AND    OR    STATIC  INSTANCEOF   SEALED  VAR IN
...
%left OR
%left AND 
%nonassoc EQUAL NOT_EQUAL CONCAT SUBARRAY
%nonassoc LESS_EQUAL GREATER_EQUAL '<' '>'
%left  '+' '-'
%left  '*' '/' '%'  
%nonassoc UMINUS '!' ARRAYREPEAT
%nonassoc '[' '.' 
%nonassoc ')' EMPTY
%nonassoc ELSE
...
Expr            :	LValue
                ...
                |   Expr ARRAYREPEAT Constant
                    {
                        $$.expr = new Tree.ArrayRepeat($1.expr, $3.typeTag, $3.literal, $2.loc);
                    }
                |   Expr CONCAT Expr
                    {
                        $$.expr = new Tree.Concat($1.expr, $3.expr, $2.loc);
                    }
                |   Expr '[' Expr SUBARRAY Expr ']'
                    {
                        $$.expr = new Tree.SubArray($1.expr, $3.expr, $5.expr, $4.loc);
                    }
